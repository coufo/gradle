...
class CompositeBuildDependencyGraphIntegrationTest extends AbstractCompositeBuildIntegrationTest {
    ...
    def setup() {
        mavenRepo.module("org.test", "buildB", "1.0").withModuleMetadata().publish()
...
"""
        ...
        checkGraph {
            edge("org.test:buildB:1.0", "project :buildB", "org.test:buildB:2.0") {
                configuration = "runtimeElements"
                compositeSubstitute()
                plainProject(":buildB:b1", "org.test:b1:2.0") { //TODO variant information is lost during substitution
                    plainProject(":buildB:b1:b11", "org.test:b11:2.0") {}
                }
            }
        }
    }
